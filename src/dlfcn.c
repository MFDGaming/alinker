...
void *android_dlopen(const char *filename, int flag)
{
    ...
    if (unlikely(ret == NULL)) {
        android_set_dlerror(DL_ERR_CANNOT_LOAD_LIBRARY);
    } else {
        ret->refcount++;
    }
...
void *android_dlsym(void *handle, const char *symbol)
{
    ...
    if(unlikely(handle == 0)) { 
        android_set_dlerror(DL_ERR_INVALID_LIBRARY_HANDLE);
        goto err;
    }
    if(unlikely(symbol == 0)) {
        android_set_dlerror(DL_ERR_BAD_SYMBOL_NAME);
        goto err;
    }
    ...
    if(likely(sym != 0)) {
        ...
        android_set_dlerror(DL_ERR_SYMBOL_NOT_GLOBAL);
    }
    else
        android_set_dlerror(DL_ERR_SYMBOL_NOT_FOUND);
